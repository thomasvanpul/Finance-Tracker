...
// Same order (Groq → Cerebras) for all three. Groq's the primary
// because of headroom (30 RPM claimed, gpt-oss-* family current);
// Cerebras is a real second (OpenAI-compatible, actually serves, 5 RPM
// but fast inference). When both fail the chain is exhausted and the
// route returns an error the user sees. There is deliberately no third
// lane.
//
// (Gemini was the tertiary until 2026-08-23: its key could never
// authenticate. OpenRouter replaced it on free models and was removed
// 2026-09-11: the free endpoints' terms forbid the financial data and
// receipt photos the chain sends. ./model-policy.ts refuses free models
// in the shared transport so neither can come back through config. See
// docs/DATA-INVENTORY.md §4.1.)
//
// If a provider's breaker is open or its key is missing, withProvider()
// throws pre-flight — the chain catches, records, moves on. No wasted
// latency on a known-dead lane. A model that failed boot verification
// is NOT skipped pre-flight: withProvider does not read modelsVerified,
// so the call is made and fails upstream instead.
//
// ── Return shape ──
...
// Today all three tasks use the same two-provider walk because Groq
// and Cerebras both support text, image, and JSON output via their
// OpenAI-compatible endpoints.
const CHAIN_ORDER: AiProviderName[] = ["groq", "cerebras"];
...
async function walk(opts: {
  ...
  for (const provider of order) {
    ...
    try {
      ...
      // errors, and openai-compat throws for a policy-refused model.
      // Chain records and continues to next lane. This is the
      // load-bearing path: never let one provider's failure fail the
      // whole request when we have others.
...
// Both current providers are OpenAI-compatible and accept a system
// message in the messages array — one translator serves both.
  ...
  for (let i = 0; i < order.length; i++) {
    ...
    try {
      const streamFn: (o: {
        ...
        maxTokens?: number;
        temperature?: number;
      }) => AsyncGenerator<OpenAiStreamChunk> = provider === "groq" ? groqChatStream : cerebrasChatStream;
...
// cheap work; falls through to Cerebras (chat model).
...
export async function chainCategorize(opts: {
  ...
  return walk({
    route: opts.route,
    attempt: async (provider) => {
      const fn = provider === "groq" ? groqCategorize : cerebrasCategorize;
...
export async function chainVision(opts: {
  ...
  return walk({
    route: opts.route,
    attempt: async (provider) => {
      const fn = provider === "groq" ? groqVision : cerebrasVision;
      return fn(opts);
    },
  });
}
