...
//
// Included:
//   • baseEquivalent — accounts, debts, upcoming, subscriptions, transactions, investments
//                      (unified 30-Aug — was split as baseEquivalent + gbpValue before the rename)
//   • *Base          — totalValueBase, totalPlBase, plBase, dayChangeBase, netBase, amountBase
//                      (matched by suffix; matches ANY identifier ending in Base)
//   • *Gbp           — legacy safety net. The 30-Aug rename sweep converted the
//                      Numeris-side Gbp identifiers to Base; leaving this alternation
//                      in the pattern means a re-introduced Gbp-suffixed money field
//                      still trips the lock rather than sneaking past.
...
const MONEY_FIELD_PATTERN = /\b(?:baseEquivalent|convertedGbp|equivalentGbp|[A-Za-z]+Gbp|[A-Za-z]+Base)\s*\?\?\s*0\b/;
    ...
    if (additions.length > 0) {
      ...
      throw new Error(
        `Found ${additions.length} new file(s) with the fabricated-zero pattern:\n${lines}\n\n` +
          `The pattern is: <identifier ending in Gbp> ?? 0, or the aliases baseEquivalent / baseEquivalent.\n` +
