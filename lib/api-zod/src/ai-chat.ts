...
//
// Design notes:
//   - text is min(1) to reject empty prompts and prevent the "empty
//     model bubble in prior history" leak (an errored/cut stream that
//     never emitted tokens leaves text: "" on the model message;
//     including that in a follow-up call silently 400'd the whole
//     request before this schema existed).
...
export const AiChatMessageSchema = z.object({
  ...
  text: z.string().min(1, "message text must not be empty").max(4000, "message text must be ≤ 4000 characters"),
});
